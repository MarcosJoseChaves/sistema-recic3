...
def registrar_rotas_contratos(blueprint, conectar_banco):
    ...
    def contratos_detalhe(contrato_id):
        ...
        try:
            ativos_contrato = AtivoService(conectar_banco).listar_do_contrato(contrato_id)
        except AtivoServiceError:
            current_app.logger.exception("Falha ao carregar ativos do contrato")
        ...
        try:
            medicoes_contrato, resumo_medicoes = MedicaoService(
                conectar_banco
            ).listar_do_contrato(contrato_id)
        except Exception:
            current_app.logger.exception("Falha ao carregar medições do contrato")
